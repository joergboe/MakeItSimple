...
`CXM_MOD_<module name>_CMI`. The value of the variable is determined in the make 
script.
...
and a Boolean value 'is_interface' (`CXM_SRC_MOD_IF_LIST`). The parts of the 
...
`CXM_MOD_<module name>_CMI` occur before the rules that require them, these 
...
The list `CXM_SRC_MOD_IF_LIST` is used to generate all grouped target rules for 
