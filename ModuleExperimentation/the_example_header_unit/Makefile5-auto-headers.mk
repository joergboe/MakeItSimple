...
# files used to generate infofiles
p1689files ::= $(addsuffix .ddi,$(CXX_$(KIND)_HEADER_UNITS))
# may be thrown away
p1689files2 ::= $(addsuffix .ddi2,$(CXX_$(KIND)_HEADER_UNITS))
# individual module map file for each unit
mapfiles ::= $(addsuffix .map,$(CXX_$(KIND)_HEADER_UNITS))

# Cleanup
tempfiles ::= $(addsuffix ~,$(infofiles))
.PHONY : clean
clean :
...
.PHONY : purge
purge :
	rm -fv stored-$(kind)-units stored-$(kind)-units-config stored-$(kind)-units-config~
	@echo
