...
ifndef silent
  ...
  $(info Sytem Header Units : $(CXXM_SYSTEM_HEADER_UNITS))
  $(info User Header Units  : $(CXXM_USER_HEADER_UNITS))
  ifdef verbose
...
$(depfiles): $(builddir_int)%.dep: % | $(call filter_out_dot_dir,$(dir $(builddir_int)%))
	@$(RM) $(verbose) '$@'
	$(depscan1)
	$(mktsimple_bindir)p1689_to_make_tab.sh '$(builddir_int)$*.ddi' '$@' '$<' '$(builddir_int)$*.o'
...
# cxx_module_request_mapper macro expands all requests to the cmi file name
# Input: req - module name list
cxx_module_request_mapper = $(foreach mm,$(req),$(CXX_MODULE_CACHE_DIR)/$(mm).$(cmi_extension))

# module_rule macro expands to a Grouped Targets Rule for a module
# Input: obj - object name
#        src - source name
#        mod - module name
#        req - requested modules list
ifdef use_clang
define module_rule =
$(obj) $(cxx_module_mapper) &: $(src) $(builddir_int)$(src).dep $(cxx_module_request_mapper) | $$(CXX_MODULE_CACHE_DIR)
	$$(if $$(silent),,@$$(RM) $$(verbose) '$(obj)' '$(cxx_module_mapper)')
	$$(CXX) -o '$(obj)' -fmodule-output='$(cxx_module_mapper)' -x c++-module '$(src)' -c\
...
$(obj) $(cxx_module_mapper) &: $(src) $(builddir_int)$(src).dep $(cxx_module_request_mapper) | $$(CXX_MODULE_CACHE_DIR)
	$$(if $$(silent),,@$$(RM) $$(verbose) '$(obj)' '$(cxx_module_mapper)')
	$$(CXX) -o '$(obj)' '$(src)' -c $$(CXXFLAGS) $$(CPPFLAGS) $$(TARGET_ARCH)
	$$(if $$(silent),,@echo -e "Finished module compiling: '$(src)'\n")
endef
endif
...
make_rules = $(foreach line,$1,\
  $(let obj src mod is_if req,$(subst ;, ,$(line)),\
    ...
    $(if $(subst -,,$(mod)),\
      $(if $(silent),,\
        $(info Module '$(mod)' : Generate Module Rule $(obj) $(cxx_module_mapper) &: $(src) $(src).dep $(cxx_module_request_mapper))\
      )\
...
modsrcs ::=
mod_if_units ::=
$(call make_rules,$(escaped_obj_src_mod_if_req_list))
